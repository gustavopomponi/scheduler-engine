...
public class TitulosDAO {
    ...
    public static List<Titulos> getTitulosVencidos(String IDCliente) {
        ...
        try (SessionFactory sessionfactory = new Configuration().configure("hibernate-sqlserver.cfg.xml").buildSessionFactory(); Session session = sessionfactory.openSession()) {
            ...
            String query = "SELECT A.*"
	                  ...
	                  +" SUBSTRING(E1.E1_VENCREA,7,2) + '/' + SUBSTRING(E1.E1_VENCREA,5,2) + '/' + SUBSTRING(E1.E1_VENCREA,1,4) AS VENC,"
                          ...
                          +" (E1.D_E_L_E_T_ <> '*') AND"
                          +" (A1.D_E_L_E_T_ <> '*') AND"
                          +" (CAST(CONVERT(VARCHAR, GETDATE(), 112) AS DATE) > CAST(E1.E1_VENCREA AS DATE)) AND"
                          +" (E1.E1_SALDO > 0) AND"
                          +" (E1.E1_CLIENTE = ?1)"
                          +" ) A";
     ...
     public static List<Titulos> getTitulosVencer(String IDCliente) {
         ...
         try (SessionFactory sessionfactory = new Configuration().configure("hibernate-sqlserver.cfg.xml").buildSessionFactory(); Session session = sessionfactory.openSession()) {
            ...
            String query = "SELECT"
	                  ...
	                  +" SUBSTRING(E1.E1_VENCREA,7,2) + '/' + SUBSTRING(E1.E1_VENCREA,5,2) + '/' + SUBSTRING(E1.E1_VENCREA,1,4) AS VENC,"
                          ...
                          +" (A1.D_E_L_E_T_ <> '*') AND"
                          +" (E1.E1_SALDO > 0) AND"
                          +" (CAST(E1.E1_VENCREA AS DATE) = CAST(CONVERT(VARCHAR, GETDATE() + 1 ,112) AS DATE)) AND"
                          +" (E1.E1_CLIENTE = ?1)";
